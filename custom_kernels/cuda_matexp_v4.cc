 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    for (int ii = 1; ii < input_num_; ii++) {
      ...
      matrixAddV4(d_mat.getData(), d_m_ii.getData(), &input_0.data()[ii], 1./div_factor, d_mat_temp.getData(), dim);
      d_mat.set(d_mat_temp.getData(),N);
    }
    ...
    for (int num = 0; num < div_-1; num++) {
      matrixMultiplication(d_mat_exp.getData(), d_mat_exp.getData(), d_mat_exp_temp.getData(), dim);
      d_mat_exp.set(d_mat_exp_temp.getData(), N);
    }

    matrixMultiplication(d_mat_exp.getData(), d_mat_exp.getData(), output.data(), dim); 
    }
    else
    {
