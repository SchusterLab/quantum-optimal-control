...
__global__ void matrixAddKernel(const float* A,const float* B, const float coeff, float* C,const int N) {
...
void matrixAdd(const float* A, const float* B, const float coeff, float* C, const int N){
    ...
    matrixAddKernel<<<blocksPerGrid,threadsPerBlock>>>(A, B, coeff, C, N);
