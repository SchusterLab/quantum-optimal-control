...
def get_reg_loss(tfs):
    ...
    with tf.name_scope('reg_errors'):
        ...
        tfs.reg_alpha_coeff = tf.placeholder(tf.float32, shape=[])
        ...
        tfs.z_reg_alpha_coeff = tf.placeholder(tf.float32, shape=[])
        z_reg_alpha = tfs.z_reg_alpha_coeff / float(tfs.sys_para.steps)
        for state in tfs.sys_para.limit_dc:
            segment_num = tfs.sys_para.limit_dc_segment_num
        ...
        tfs.dwdt_reg_alpha_coeff = tf.placeholder(tf.float32, shape=[])
        ...
        tfs.d2wdt2_reg_alpha_coeff = tf.placeholder(tf.float32, shape=[])
        ...
        tfs.inter_reg_alpha_coeff = tf.placeholder(tf.float32, shape=[])
