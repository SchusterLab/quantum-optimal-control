    ...
    with tf.name_scope('reg_errors'):
        ...
        if 'd2wdt2' in tfs.sys_para.reg_coeffs:
            ...
            reg_loss = reg_loss + d2wdt2_reg_alpha * tf.nn.l2_loss((new_weights[:, 2:] -
                                                                    2 * new_weights[:, 1:tfs.sys_para.steps + 3] +
                                                                    new_weights[:, :tfs.sys_para.steps + 2]) /
                                                                   (tfs.sys_para.dt ** 2))
        ...
        if 'bandpass' in tfs.sys_para.reg_coeffs:
            ...
            bandpass_reg_alpha = bandpass_reg_alpha_coeff / \
                float(tfs.sys_para.steps)
        ...
        if 'forbidden_coeff_list' in tfs.sys_para.reg_coeffs:
            ...
            for inter_vec in tfs.inter_vecs:
                ...
                for inter_reg_alpha_coeff, state in zip(tfs.sys_para.reg_coeffs['forbidden_coeff_list'], tfs.sys_para.reg_coeffs['states_forbidden_list']):
                    ...
                    reg_loss = reg_loss + inter_reg_alpha * \
                        tf.nn.l2_loss(forbidden_state_pop)

        # Speeding up the gate time
        if 'speed_up' in tfs.sys_para.reg_coeffs:
            speed_up_reg_alpha_coeff = tfs.sys_para.reg_coeffs['speed_up']
            speed_up_reg_alpha = speed_up_reg_alpha_coeff / \
                float(tfs.sys_para.steps)

            target_vecs_all_timestep = tf.tile(tf.reshape(tfs.target_vecs, [
                                               2*tfs.sys_para.state_num, 1, len(tfs.inter_vecs)]), [1, tfs.sys_para.steps+1, 1])

            target_vecs_inner_product = tfs.get_inner_product_3D(
                tfs.inter_vecs_packed, target_vecs_all_timestep)
            reg_loss = reg_loss + speed_up_reg_alpha * \
                tf.nn.l2_loss(tfs.sys_para.steps+1 - target_vecs_inner_product)

        if 'endpoints_zero' in tfs.sys_para.reg_coeffs:
            zeros_coeff = tfs.sys_para.reg_coeffs['endpoints_zero']
            for ii in range(tfs.ops_weight.get_shape().as_list()[0]):
                reg_loss += zeros_coeff * (tf.abs(tfs.ops_weight[ii, 0]) + tf.abs(tfs.ops_weight[ii, -1]))

        if 'integral_zero' in tfs.sys_para.reg_coeffs:
            int_coeff = tfs.sys_para.reg_coeffs['integral_zero']
            for ii in range(tfs.ops_weight.get_shape().as_list()[0]):
                reg_loss += int_coeff * tf.abs(tf.reduce_sum(tfs.ops_weight[ii]))

        if 'max_drive' in tfs.sys_para.reg_coeffs:
            max_coeff = tfs.sys_para.reg_coeffs['max_drive']
            reg_loss += tf.abs(tf.reduce_sum(tf.square(tfs.ops_weight[0]) + tf.square(tfs.ops_weight[1]) -
                                             1.0))
